...
namespace TradingBot {
    ...
    template <class TraderType>
    class AutoFitTrader : public Trader {
    public:
        AutoFitTrader(Strategy* strategy, Market* market, const ParamSet& paramSet, const ParamSet& paramSetMin, const ParamSet& paramSetMax);
        AutoFitTrader(
    ...
    AutoFitTrader<Strat>::AutoFitTrader(
    ...
    template <class Strat>
    std::vector<std::vector<std::pair<time_t, double> > > AutoFitTrader<Strat>::getPlots() {
    ...
    template <class Strat>
    Signal AutoFitTrader<Strat>::step(bool newCandle) {
        if (!newCandle) {
            return {};
        }
    ...
    template <class Strat>
    bool AutoFitTrader<Strat>::checkParamSet(const ParamSet& paramSet) const {
        if (paramSet.size() != 6) {
            return false;
        }
